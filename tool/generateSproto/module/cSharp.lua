...
local function constructor_protocol(class, package, stream, deep)
  ...
  deep = deep + 1
    for _,class_info in ipairs(class) do
      ...
      -- local stag = type_namespace.."."..name..".Tag"

      stream:write("Protocol.SetProtocol<"..type_namespace.."."..name.."> ("..tag..");", deep)
      
      if request_type then
        request_type = type_namespace.."."..request_type
        stream:write("Protocol.SetRequest<"..request_type.."> ("..tag..");",deep)
      end

      if response_type then
        response_type = type_namespace.."."..response_type
        stream:write("Protocol.SetResponse<"..response_type.."> ("..tag..");", deep)
      end
      stream:write()
    end
  deep = deep - 1
