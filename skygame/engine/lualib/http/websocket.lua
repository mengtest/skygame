...
local function _new_server_ws(socket_id, handle, protocol)
    ...
    elseif protocol == "wss" then
        ...
        skynet.error(string.format("invalid websocket protocol:%s", tostring(protocol)))
        return
    end
...
function M.accept(socket_id, handle, protocol, addr)
    ...
    if not ws_obj then
        skynet.error("accept error, ws_obj is nil")
        return
    end
    ws_obj.addr = addr
...
function M.write(id, data, fmt, masking_key)
    if(_isws_closed(id)) then
        skynet.error("ws write error, id is nil.")
        return
    end
    local ws_obj = assert(ws_pool[id])
    fmt = fmt or "text"
