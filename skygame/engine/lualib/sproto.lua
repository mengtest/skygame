...
function host:dispatch2(...)
	...
	if header.type then
		...
		local msgdata
		if proto.request then
			msgdata = core.decode(proto.request, content)
		end
		-- if header_tmp.session then
		-- 	return "REQUEST", proto.name, result, gen_response(self, proto.response, header_tmp.session), header.ud
		-- else
		-- 	return "REQUEST", proto.name, result, nil, header.ud
		-- end
		return proto, msgdata, header
	end
end

function host:getresponse(response, session)
	return gen_response(self, response, session)
end
--EOD 2
