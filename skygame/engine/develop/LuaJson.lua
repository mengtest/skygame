...
local function json2number(str, from, to)
    ...
    if not num then
        WARN("read json格式错误，不正确的数字, 错误位置:", from, str)
    end
    return num, i - 1
end
...
local function json2string(str, from, to)
    ...
    WARN("read json格式错误，字符串没有找到结尾, 错误位置:", from, str)
end
...
local function json2array(str, from, to)
    ...
    WARN("read json格式错误，表没有找到结尾, 错误位置:", from, str)
end
